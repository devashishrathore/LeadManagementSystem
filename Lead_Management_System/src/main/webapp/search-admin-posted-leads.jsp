...
int totalLeadCountByAdded = leadDao.getLeadsCountUsingCompanyIdAndStatus(companyId, "Added in Batch");int totalLeadCountByPosted = leadDao.getLeadsCountUsingPosted(userCookie.getEmail(), companyId);
...
if (searchBy.equals("id")) {
	...
	leadCount = leadDao.getPostedLeadsCountByCompanyId("SELECT COUNT(id) FROM leads WHERE email LIKE ? AND companyid=? AND owner=?",
	search, companyId, userCookie.getEmail());
} else if (searchBy.equals("address")) {
	leadCount = leadDao.getPostedLeadsCountByCompanyId("SELECT COUNT(id) FROM leads WHERE address LIKE ? AND companyid=? AND owner=?",
	search, companyId, userCookie.getEmail());
} else if (searchBy.equals("name")) {
	leadCount = leadDao.getPostedLeadsCountByCompanyId("SELECT COUNT(id) FROM leads WHERE name LIKE ? AND companyid=? AND owner=?",
	search, companyId, userCookie.getEmail());
} else if (searchBy.equals("mobile")) {
	leadCount = leadDao.getPostedLeadsCountByCompanyId("SELECT COUNT(id) FROM leads WHERE mobile LIKE ? AND companyid=? AND owner=?",
	search, companyId, userCookie.getEmail());
} else if (searchBy.equals("owner")) {
	leadCount = leadDao.getPostedLeadsCountByCompanyId("SELECT COUNT(id) FROM leads WHERE owner LIKE ? AND companyid=? AND owner=?",
	search, companyId, userCookie.getEmail());
} else if (searchBy.equals("status")) {
	leadCount = leadDao.getPostedLeadsCountByCompanyId("SELECT COUNT(id) FROM leads WHERE status LIKE ? AND companyid=? AND owner=?",
	search, companyId, userCookie.getEmail());
} else if (searchBy.equals("currentowner")) {
	leadCount = leadDao.getPostedLeadsCountByCompanyId(
	"SELECT COUNT(id) FROM leads WHERE currentowner LIKE ? AND companyid=? AND owner=?", search, companyId, userCookie.getEmail());
}
...
<body>
	...
	<div id="main">
		...
		<div class="main-container p-2">
			<form action="/Lead_Management_System/search-admin-posted-leads.jsp"
				method="get" class="">
				<div class="row">
		...
		<div class="main-container">
			...
			<div>
				...
				<a class='submit-btn w-100'
					...
					href="/Lead_Management_System/search-admin-posted-leads.jsp?searchby=<%=searchBy%>&search=<%=search %>&page=<%=currentPage - 1%>">
					&lt; Previous</a>
				<%
				}
				%>
				...
				<a class='submit-btn w-100'
					style="padding: 2px 4px; text-decoration: none;"
					href="/Lead_Management_System/search-admin-posted-leads.jsp?searchby=<%=searchBy%>&search=<%=search %>&page=<%=i%>"><%=i%></a>
				<%
				}
				}
				%>
				...
				<a class='submit-btn w-100'
					style="padding: 2px 4px; text-decoration: none;"
					href="/Lead_Management_System/search-admin-posted-leads.jsp?page=searchby=<%=searchBy%>&search=<%=search %>&<%=currentPage + 1%>">Next
					&gt;</a>
				<%
				}
				%>
