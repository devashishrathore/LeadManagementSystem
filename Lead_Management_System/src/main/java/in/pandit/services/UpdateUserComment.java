...
public class UpdateUserComment extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(Objects.nonNull(comments)) {
			...
			session.setAttribute("user1Msg", "Comment Updated");
			response.sendRedirect("show-user-lead-comment.jsp?leadid="+comments.getLeadid());
		}else {
			session.setAttribute("user1Msg", "Something went wrong!");
			response.sendRedirect("show-user-lead-comment.jsp");
		}
	}
