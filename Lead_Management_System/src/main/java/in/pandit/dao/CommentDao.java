...
public class CommentDao {
	...
	private static final String GET_All_COMMENT_BY_USER_ID_OFFSET="SELECT * FROM comments WHERE userid=? ORDER BY creation_date DESC LIMIT ? OFFSET ?";
	private static final String GET_All_COMMENT_BY_COMPANY_ID="SELECT * FROM comments WHERE companyid=? ORDER BY creation_date DESC";
	private static final String GET_All_COMMENT_BY_COMPANY_ID_OFFSET="SELECT * FROM comments WHERE companyid=? ORDER BY creation_date DESC LIMIT ? OFFSET ?";
	...
	public List<Comment> getAllCommentByCompanyId(int companyid) {
		List<Comment> list= new ArrayList<>();
		try {
			pst = con.prepareStatement(GET_All_COMMENT_BY_COMPANY_ID);
			pst.setInt(1, companyid);
			ResultSet rst = pst.executeQuery();
			while(rst.next()) {
				Comment comment = new Comment();
	...
	public List<Comment> getAllCommentByCompanyIdOffset(int companyid, int limit, int offset) {
		List<Comment> list= new ArrayList<>();
		try {
			pst = con.prepareStatement(GET_All_COMMENT_BY_COMPANY_ID_OFFSET);
			pst.setInt(1, companyid);
			pst.setInt(2, limit);
			pst.setInt(3, offset);
			ResultSet rst = pst.executeQuery();
...
//				comment.setCreationDate(rst.getDate("creation_date"));
	...
	public List<Comment> getAllCommentByUserIdOffset(int userId, int limit, int offset) {
		List<Comment> list= new ArrayList<>();
		try {
			pst = con.prepareStatement(GET_All_COMMENT_BY_USER_ID_OFFSET);
			pst.setInt(1, userId);
			pst.setInt(2, limit);
			pst.setInt(3, offset);
			ResultSet rst = pst.executeQuery();
