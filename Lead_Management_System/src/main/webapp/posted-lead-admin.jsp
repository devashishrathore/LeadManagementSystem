...
int totalPostedLeads = leadDao.getLeadsCountUsingPosted(userCookie.getEmail(), companyId);
...
int itemsPerPage = 20;
int totalPages = (int) Math.ceil((double) totalPostedLeads / itemsPerPage);
	...
	<div id="main">
		...
		<div class="pe-2 ps-2">
			<p class="fs-2 text-white box-heading"><%if(list.size() <= 0) {
				...
				}%>All Posted Leads (Total results: <%= totalPostedLeads %>, Page <%= currentPage %>/<%=totalPages%>)</p>
		</div>
		<hr class="divide">		
...
<div>
					...
					<a class='submit-btn w-100'
						...
						href="/Lead_Management_System/posted-lead-admin.jsp?page=<%=currentPage - 1%>">
						&lt; Previous</a>
					<%
					}
					%>
					...
					if(endPage!=1){
						for (int i = startPage; i <= endPage; i++) {
							...
							<a class='submit-btn w-100'
								style="padding: 2px 4px; text-decoration: none;"
								href="/Lead_Management_System/posted-lead-admin.jsp?page=<%=i%>"><%=i%></a>
							<%
							}
					}
					%>
					...
					<a class='submit-btn w-100'
						style="padding: 2px 4px; text-decoration: none;"
						href="/Lead_Management_System/posted-lead-admin.jsp?page=<%=currentPage + 1%>">Next
						&gt;</a>
					<%
					}
					%>
