...
int itemsPerPage = 20;
int totalPages = (int) Math.ceil((double) totalLeadCountByAssigned / itemsPerPage);
List<Lead> list = leadDao.getAllLeadsByLimitOffsetAndCompanyAndCurrentOwner(itemsPerPage, (currentPage - 1) * itemsPerPage, companyId,userCookie.getEmail());
%>
<!Doctype HTML>
<html>
<head>
	...
	<div id="main">
		...
		<div class="pe-2 ps-2">
			<p class="fs-2 text-white box-heading"><%if(list.size() <= 0) {
				totalLeadCountByAssigned =0; currentPage=0;
				}%>All Assigned Leads (Total Results: <%= totalLeadCountByAssigned %>, Page <%= currentPage %>/<%=totalPages%>)</p>
		</div>
		<hr class="divide">		
			...
			<div class="p-2">
				...
				<%
					...
					<a class='submit-btn w-100'
						...
						href="/Lead_Management_System/assigned-lead-admin.jsp?page=<%=currentPage - 1%>">
						&lt; Previous</a>
					<%
					}
					%>
					...
					if(endPage!=1){
						for (int i = startPage; i <= endPage; i++) {
							...
							<a class='submit-btn w-100'
								style="padding: 2px 4px; text-decoration: none;"
								href="/Lead_Management_System/assigned-lead-admin.jsp?page=<%=i%>"><%=i%></a>
							<%
							}
					}
					%>
					...
					<a class='submit-btn w-100'
						style="padding: 2px 4px; text-decoration: none;"
						href="/Lead_Management_System/assigned-lead-admin.jsp?page=<%=currentPage + 1%>">Next
						&gt;</a>
					<%
					}
					%>
