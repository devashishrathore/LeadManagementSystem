...
int totalPostedLeadCount = leadDao.getLeadsCountUsingPosted(userCookie.getEmail(),companyId);
int totalLeadCountBySource = leadDao.getLeadsCountUsingAssigned(userCookie.getEmail(), companyId);
Connection connect = DatabaseConnection.getConnection();
int currentPage = (request.getParameter("page") != null) ? Integer.parseInt(request.getParameter("page")) : 1;
int itemsPerPage = 20;
int totalPages = (int) Math.ceil((double) totalPostedLeadCount / itemsPerPage);
List<Lead> list = leadDao.getAllLeadsByLimitOffsetAndCompanyAndOwner(itemsPerPage, (currentPage - 1) * itemsPerPage,
		companyId, userCookie.getEmail());
%>
<!Doctype HTML>
<html>
<head>
...
<body>
	...
	<div id="main">
		...
		<div class="main-container p-2">
			<form action="/Lead_Management_System/search-user-posted-leads.jsp"
				method="get">
				<div class="row">
		...
		<div class="pe-2 ps-2">
			<p class="fs-2 text-white box-heading"><%if(list.size() <= 0) {
				totalPostedLeadCount=0; currentPage=0;
				}%>All Posted Leads (Total Results: <%= totalPostedLeadCount %>, Page <%= currentPage %>/<%=totalPages%>)</p>
		</div>
		<hr class="divide">
		<div class="main-container">
			...
			<div class="p-2 w-100 text-white">
				...
				<a href="dashboard.jsp" class='submit-btn w-100 '
			...
			<div class="p-2">
				...
				<div>
					...
					<a class='submit-btn w-100'
						...
						href="/Lead_Management_System/posted-lead-user.jsp?page=<%=currentPage - 1%>">
						&lt; Previous</a>
					<%
					}
					%>
					...
					if(endPage!=1){
						for (int i = startPage; i <= endPage; i++) {
							...
							<a class='submit-btn w-100'
								style="padding: 2px 4px; text-decoration: none;"
								href="/Lead_Management_System/posted-lead-user.jsp?page=<%=i%>"><%=i%></a>
							<%
							}
					}
					%>
					...
					<a class='submit-btn w-100'
						style="padding: 2px 4px; text-decoration: none;"
						href="/Lead_Management_System/posted-lead-user.jsp?page=<%=currentPage + 1%>">Next
						&gt;</a>
					<%
					}
					%>
