...
List<Lead> list = leadDao.getUserLeadsByLimitOffsetAndCompanyAndStatus(itemsPerPage, (currentPage - 1) * itemsPerPage, companyId,"Interview",userCookie.getEmail());
%>
<!Doctype HTML>
<html>
<head>
	...
	<div id="main">
		...
		<div class="pe-2 ps-2">
			<p class="fs-2 text-white box-heading"><%if(list.size() > 0) { %>
				All Interviews (Total Results: <%= totalLeadCountByInterview %>, Page <%= currentPage %>/<%=totalPages%>)</p>
		<%} %>
		</div>
		<hr class="divide">		
		...
		<%if(list.size()<=0){%>
			<div class="p-2 w-100 text-white">
				...
				<a href="dashboard.jsp" class = 'submit-btn w-100 ' style="text-decoration: none;">Add Interview</a>
			</div>
			<%}else{%>
			<div class="p-2">
				<%try{
				...
				<%
					...
					<a class='submit-btn w-100'
						...
						href="/Lead_Management_System/show-all-interview.jsp?page=<%=currentPage - 1%>">
						&lt; Previous</a>
					<%
					}
					%>
					...
					if(endPage!=1){
						for (int i = startPage; i <= endPage; i++) {
							...
							<a class='submit-btn w-100'
								style="padding: 2px 4px; text-decoration: none;"
								href="/Lead_Management_System/show-all-interview.jsp?page=<%=i%>"><%=i%></a>
							<%
							}
					}
					%>
					...
					<a class='submit-btn w-100'
						style="padding: 2px 4px; text-decoration: none;"
						href="/Lead_Management_System/show-all-interview.jsp?page=<%=currentPage + 1%>">Next
						&gt;</a>
					<%
					}
					%>
